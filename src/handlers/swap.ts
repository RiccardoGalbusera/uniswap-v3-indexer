...
UniswapV3Pool.Swap.handlerWithLoader({
  ...
  handler: async ({ event, context, loaderReturn }) => {
    ...
    const uniswapDayData = {
      ...(await intervalUpdates.updateUniswapDayData(
        timestamp,
        event.chainId,
        factory,
        context
    ...
    const token0DayData = {
      ...(await intervalUpdates.updateTokenDayData(
        timestamp,
        token0,
        bundle,
        context
    ...
    const token0HourData = {
      ...(await intervalUpdates.updateTokenHourData(
        timestamp,
        token0,
        bundle,
        context
